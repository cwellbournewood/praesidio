# Praesidio JetBrains plugin — changelog
...
### Added
- Initial release of the Praesidio JetBrains plugin.
...
- Praesidio tool window on the right edge: gateway / sign-in / proxy
...
- "Toggle Edge Proxy" action that spawns `praesidio-edge-proxy` as a
  managed child process and kills it cleanly on IDE shutdown.
- Settings panel under **Tools → Praesidio** with API key, OIDC sign-in,
