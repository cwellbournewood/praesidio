 ...
 * issuer is discovered from `{gateway}/.well-known/praesidio-edge`.
 *
 * On success:
...
class SignInController(
    ...
    private fun title(): String = message("praesidio.action.signin.title")
    ...
    private fun runFlow(indicator: ProgressIndicator): Boolean {
        ...
        ApplicationManager.getApplication().invokeLater {
            ...
            Messages.showInfoMessage(
                message(
                    "praesidio.action.signin.deviceCode",
            ...
            when (token) {
                is TokenPollResult.Success -> {
                    ...
                    notify(
                        null,
                        message(
                            "praesidio.action.signin.success",
                            PraesidioSettings.getInstance().signedInAs,
                        ),
    ...
    private fun fetchDiscovery(gateway: String): Discovery {
        val req = Request.Builder()
            .url("$gateway/.well-known/praesidio-edge")
            .get()
    ...
    private fun requestDeviceCode(d: Discovery): DeviceAuthResponse {
        val body = FormBody.Builder()
            .add("client_id", d.clientId ?: "praesidio-edge")
    ...
    private fun storeCredentials(
        settings: PraesidioSettings,
        gateway: String,
        token: TokenPollResult.Success,
        d: Discovery,
    ) {
        val store = PraesidioCredentialStore(gateway)
    ...
    private fun notifyError(text: String?) {
        notify(
            null,
            message("praesidio.action.signin.error", text ?: "unknown error"),
