...
/**
 * "Section: Scan Selection" — sends the current editor selection
...
class ScanSelectionAction : AnAction() {
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        if (selection.isNullOrEmpty()) {
            notify(
                project,
                message("section.action.scan.empty"),
    ...
    private fun notifyScan(project: Project?, resp: ScanResponse) {
        when {
            ...
            resp.isMask ->
                notify(
                    project,
                    message("section.action.scan.mask", resp.findings.size),
                    NotificationType.WARNING,
                )
            else ->
                notify(
    ...
    companion object {
        ...
        const val NOTIFY_GROUP: String = "Section"
    }
}
