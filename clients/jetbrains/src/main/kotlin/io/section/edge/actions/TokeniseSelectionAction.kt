...
/**
 * "Section: Tokenise Selection" — scans the selection and, if the
...
class TokeniseSelectionAction : AnAction() {
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        if (selection.isNullOrEmpty()) {
            notify(
                project,
                message("section.action.tokenise.nothing"),
                NotificationType.WARNING,
            )
            return
        }

        object : Task.Backgroundable(project, message("section.action.scan.title"), true) {
    ...
    private fun runTokenise(
        ...
        when (val result = client.scan(req)) {
            ...
            is GatewayResult.Err ->
                notify(
                    project,
                    message("section.action.scan.error", result.message),
    ...
    private fun applyResult(
        ...
        if (resp.isBlock) {
            notify(
                project,
                message("section.action.tokenise.blocked"),
                NotificationType.ERROR,
            )
            return
        }
        ...
        if (resp.isAllow || sanitised == null ||
            ...
            notify(
                project,
                message("section.action.scan.allow"),
        ...
        ApplicationManager.getApplication().invokeLater {
            ...
            notify(
                project,
                message("section.action.tokenise.replaced", resp.transforms.size),
                NotificationType.INFORMATION,
            )
        }
    }
