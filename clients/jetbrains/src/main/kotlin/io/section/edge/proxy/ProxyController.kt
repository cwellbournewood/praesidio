 ...
 * `section-edge-proxy` CLI (Lane E). One process per IDE — multiple
...
class ProxyController : Disposable {
     ...
     */
    internal fun buildCommand(settings: SectionSettings): List<String> {
    ...
    private fun pumpOutput(proc: Process) {
        ...
        }, "section-edge-proxy-output").apply { isDaemon = true }.start()
    }
    ...
    companion object {
        const val DEFAULT_BINARY: String = "section-edge-proxy"
        ...
        val TOPIC: Topic<Listener> =
            Topic.create("Section.ProxyState", Listener::class.java)
