...
/**
 * Persistent, application-level settings for the Section plugin.
 *
 * Stored in `<config-dir>/options/section.xml`. Sensitive material
 * (API key, OIDC refresh token) lives in [SectionCredentialStore]
...
@State(
    ...
    storages = [Storage("section.xml")],
)
@Service(Service.Level.APP)
class SectionSettings : PersistentStateComponent<SectionSettings.State> {
    ...
    class State {
        ...
        /**
         * User id surfaced to the gateway as `X-Section-User` when
    ...
    /**
     * Commit fresh state — called by [SectionConfigurable.apply].
    ...
    companion object {
        ...
        @JvmStatic
        fun getInstance(): SectionSettings =
            ApplicationManager.getApplication().getService(SectionSettings::class.java)
    }
}
