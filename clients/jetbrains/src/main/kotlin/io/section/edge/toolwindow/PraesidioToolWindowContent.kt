...
import io.section.edge.gateway.RecentDecisionsService
import io.section.edge.proxy.ProxyController
import io.section.edge.settings.SectionConfigurable
 ...
 */
class SectionToolWindowContent(
    ...
    private val toggleProxyBtn = JButton(message("section.toolwindow.button.toggleProxy"))
    private val settingsBtn = JButton(message("section.toolwindow.button.openSettings"))
    private val signInBtn = JButton(message("section.toolwindow.button.signIn"))
    ...
    private fun refreshStatus() {
        ...
        statusGateway.text = message(
            "section.toolwindow.status.gateway",
            settings.gatewayUrl,
        )
        statusSignIn.text = if (settings.signedInAs.isNotEmpty()) {
            message("section.toolwindow.status.signedIn", settings.signedInAs)
        } else {
            message("section.toolwindow.status.signedOut")
        }
        val proxyLabel = when (ProxyController.getInstance().state) {
            ProxyController.ProxyState.RUNNING ->
                message("section.toolwindow.status.proxy.running")
            else ->
                message("section.toolwindow.status.proxy.stopped")
        }
        statusProxy.text = message("section.toolwindow.status.proxy", proxyLabel)
    }
    ...
    private fun handleToggleProxy(): ActionListener = ActionListener {
        ...
        val action: AnAction? = mgr.getAction("io.section.edge.actions.ToggleProxyAction")
        if (action != null) {
    ...
    private fun handleOpenSettings(): ActionListener = ActionListener {
        ShowSettingsUtil.getInstance()
            .showSettingsDialog(project, SectionConfigurable::class.java)
    }
    ...
    private class DecisionsTableModel : AbstractTableModel() {
        ...
        private val columns = arrayOf(
            message("section.toolwindow.recent.column.time"),
            message("section.toolwindow.recent.column.action"),
            message("section.toolwindow.recent.column.findings"),
            message("section.toolwindow.recent.column.client"),
        )

        fun prepend(decision: RecentDecisionsService.Decision) {
            rows.add(0, decision)
            // Keep the model in sync with the service capacity.
            val cap = SectionSettings.getInstance().recentDecisionsLimit
