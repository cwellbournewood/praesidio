...
class GatewayClientTest {
    ...
    fun `scan sends API key, tenant, and groups headers`() {
        ...
        assertThat(recorded.getHeader("X-Praesidio-Tenant")).isEqualTo("acme")
        assertThat(recorded.getHeader("X-Praesidio-User")).isEqualTo("alice@acme.example")
        assertThat(recorded.getHeader("X-Praesidio-Groups")).isEqualTo("dev,security")
    ...
    fun `scan with null model omits the field entirely`() {
        ...
        // field absent so the gateway uses its own default "praesidio-edge".
        server.enqueue(MockResponse().setBody(allowResponse()))
    ...
    fun `client header marks edge source`() {
        ...
        assertThat(recorded.getHeader("X-Praesidio-Client")).isEqualTo("jetbrains")
    }
