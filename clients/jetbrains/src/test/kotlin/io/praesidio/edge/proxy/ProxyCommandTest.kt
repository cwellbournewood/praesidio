...
class ProxyCommandTest {
    ...
    @Test
    fun `default command uses praesidio-edge-proxy on PATH`() {
        val cmd = reproduceBuildCommand(
            binaryPath = "",
            gatewayUrl = "https://gw.example/",
            proxyArgs = listOf("start"),
        )
        assertThat(cmd.first()).isEqualTo("praesidio-edge-proxy")
    ...
    fun `custom binary path is honoured`() {
        val cmd = reproduceBuildCommand(
            binaryPath = "/usr/local/bin/praesidio-edge-proxy",
            gatewayUrl = "https://gw.example/",
            proxyArgs = listOf("start"),
        )
        assertThat(cmd.first()).isEqualTo("/usr/local/bin/praesidio-edge-proxy")
    }
