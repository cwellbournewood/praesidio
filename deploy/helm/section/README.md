...
Production-shaped chart for the Section AI Security Control Plane
(gateway + admin UI, with optional embedded Postgres / Redis for dev).

## TL;DR

```bash
helm repo add section https://cwellbournewood.github.io/section   # (when published)
helm install section section/section \
...
|---|---|---|
| `image.gateway.repository` | `ghcr.io/section/gateway` | Gateway image |
| `image.ui.repository` | `ghcr.io/section/ui` | UI image |
