# EU AI Act — Section control mapping
...
that Section's customers will typically have. Section itself is not an AI
system in the Act's sense; it is a governance control surface that supports
those obligations.

| Article | Obligation (paraphrased) | Section control |
|---|---|---|
...
| Art. 26 (Deployer obligations) | Use as intended, monitor operation, human oversight, input data control | Section *is* the input data control layer; outputs/logs power the monitoring. |
...
| Art. 53 (GPAI providers) | Technical documentation, copyright policy, training summary | n/a — Section is a deployer-side control, but it records which GPAI model was used per request. |
