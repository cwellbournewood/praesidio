...
Directory) as the identity provider for the Section admin UI and the
...
2. Fill in:
   ...
   - **Redirect URI**: platform `Web`,
     `https://ui.section.example/api/auth/callback/azure-ad`.
...
* Under **Implicit grant and hybrid flows**, enable **ID tokens (used
  ...
  Implicit grant.
* **Front-channel logout URL**: `https://ui.section.example/api/auth/signout`
...
This is where Entra trips up most operators. Section needs **group
...
3. Pick `tid` (Azure tenant) — this gives every token a stable Azure
   tenant ID that Section maps to its own tenant via
   `SECTION_OIDC_TENANT_CLAIM=tid`.
4. (Optional) For a richer multi-tenancy model where you have multiple
   Section tenants inside one Entra tenant, store a custom attribute
...
names) into `SECTION_RBAC_GROUP_MAP`. Example with GUIDs:

```ini
SECTION_RBAC_GROUP_MAP={"d34db33f-…-admins":["admin"],"d34db33f-…-ops":["operator","viewer"],"d34db33f-…-auditors":["auditor","viewer"],"d34db33f-…-viewers":["viewer"]}
```
...
OIDC_REDIRECT_URI=https://ui.section.example/api/auth/callback/azure-ad
...
REDIRECT=https://ui.section.example/api/auth/callback/azure-ad
...
This caps the claim at the groups directly assigned to the Section app
...
5. UI exchanges code, stores session cookie, forwards verified identity
   to the gateway as `X-Section-User`, `X-Section-Groups`,
   `X-Section-Tenant` headers (signed by the reverse-proxy).
