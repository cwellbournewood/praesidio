#!/usr/bin/env bash
# Generate development-quality random values for the Praesidio token vault
...
# PRAESIDIO_VAULT_KEY: 32 bytes, base64
vault_key=$(openssl rand -base64 32)

# PRAESIDIO_FPE_KEY: FF3-1 takes 16, 24, or 32 bytes (we generate 32 = AES-256), hex
fpe_key=$(openssl rand -hex 32)

# PRAESIDIO_FPE_TWEAK: 56 bits = 7 bytes for FF3-1, hex
...
# -----------------------------------------------------------------------------
PRAESIDIO_VAULT_KEY=${vault_key}
PRAESIDIO_FPE_KEY=${fpe_key}
PRAESIDIO_FPE_TWEAK=${fpe_tweak}
EOF
