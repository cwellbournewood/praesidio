"""Runtime configuration for the Praesidio edge proxy.

Loaded from environment variables (``PRAESIDIO_EDGE_*``) and CLI flags.
...
class EdgeSettings(BaseSettings):
    ...
    Environment-variable prefix: ``PRAESIDIO_EDGE_``. All fields can be
    overridden by CLI flags via :func:`praesidio_edge_proxy.cli.build_settings`.
    """

    model_config = SettingsConfigDict(
        env_prefix="PRAESIDIO_EDGE_",
        env_file=None,
    ...
    gateway_url: str = Field(
        ...
        description="Praesidio gateway base URL (no trailing /v1).",
    )
    api_key: str = Field(
        default="",
    ...
    tenant: str = Field(
        default="default",
        description="Tenant id presented as `X-Praesidio-Tenant`.",
    )
    listen_host: str = Field(default="127.0.0.1")
    listen_port: int = Field(default=8888, ge=1, le=65535)

    # CA / install paths — see :mod:`praesidio_edge_proxy.ca` for defaults.
    ...
    session_id_header: str = Field(
        default="x-praesidio-session-id",
