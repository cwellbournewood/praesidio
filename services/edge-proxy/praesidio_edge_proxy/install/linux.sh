#!/bin/sh
# Praesidio Edge Proxy — Linux trust-store install.
#
# Usage:
...
INSTALLED="$ANCHOR_DIR/praesidio-edge-proxy.crt"

if [ "$MODE" = "--uninstall" ]; then
    echo "Removing Praesidio CA from $ANCHOR_DIR..."
    rm -f "$INSTALLED"
    if ! $UPDATE_CMD; then
...
echo "Installing Praesidio CA into $ANCHOR_DIR..."
cp "$CERT" "$INSTALLED"
