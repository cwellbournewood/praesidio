...
<#
    Praesidio Edge Proxy — Windows trust-store install.
...
if ($Mode -eq "--uninstall") {
    ...
    Write-Host "Removing Praesidio CA from LocalMachine\Root..."
    $existing = Get-ChildItem -Path Cert:\LocalMachine\Root |
        Where-Object { $_.Subject -like "*Praesidio Edge Proxy Local CA*" }
    if ($null -eq $existing) {
    ...
    Write-Host "OK: Praesidio CA removed from trust store."
    exit 0
}

Write-Host "Installing Praesidio CA into LocalMachine\Root..."
& certutil.exe -addstore -f Root $CertPath | Out-Null
if ($LASTEXITCODE -ne 0) {
    Write-Host "ERROR: certutil -addstore failed (exit $LASTEXITCODE)"
    exit 2
}
Write-Host "OK: Praesidio CA installed."
exit 0
