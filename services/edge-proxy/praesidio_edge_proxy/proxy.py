...
This module is loaded as an addon by mitmproxy via :mod:`praesidio_edge_proxy.cli`.
It exposes :class:`PraesidioAddon` whose ``request`` and ``response``
hooks are called for every flow. We:

1. Skip anything not in :mod:`praesidio_edge_proxy.upstream`.
2. On request, extract the prompt, call ``/v1/scan``, and either:
   - block with HTTP 403 (`praesidio_blocked` body matching the gateway's shape),
...
class PraesidioAddon:
    ...
    async def request(self, flow: Any) -> None:
        ...
        try:
            ...
            if not self.settings.fail_open:
                self._respond_error(flow, 502, "praesidio_gateway_unreachable", str(exc))
            return
    ...
    async def response(self, flow: Any) -> None:
        ...
        state: FlowState | None = flow.metadata.get("praesidio") if hasattr(flow, "metadata") else None
