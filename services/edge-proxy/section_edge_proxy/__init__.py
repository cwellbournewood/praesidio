"""Section Edge Proxy — local CA MITM for LLM-API traffic.
...
* For each intercepted request, extracts the prompt text from the
  provider-specific JSON shape and POSTs it to the Section gateway's
  ``/v1/scan`` endpoint.
* On ``action="block"`` returns HTTP 403 to the caller with the gateway's
  standard ``section_blocked`` error body.
