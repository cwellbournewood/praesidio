...
```
ca/
├── section-ca.crt    # public root cert; safe to install in trust store
├── section-ca.key    # PRIVATE key — never shipped, never logged
...
* :func:`install_command` — return the per-OS shell command + script
  path used by :mod:`section_edge_proxy.cli` to add the root to the
  trust store.
...
def default_ca_dir() -> Path:
    ...
    * Windows: ``%LOCALAPPDATA%\\Section``
    * macOS: ``~/Library/Application Support/Section``
    * Linux: ``$XDG_DATA_HOME/section`` (falls back to ``~/.local/share/section``)
    """
    ...
    if sysname == "Darwin":
        return Path.home() / "Library" / "Application Support" / "Section"
    # Linux & everyone else.
...
def ca_paths(ca_dir: Path | None = None) -> tuple[Path, Path]:
    ...
    return base / "section-ca.crt", base / "section-ca.key"
...
_KEY_SIZE_BITS = 4096
_VALID_DAYS = 365 * 10
_CN = "Section Edge Proxy Local CA"
_ORG = "Section"
