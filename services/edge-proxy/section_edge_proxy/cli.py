...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="section-edge-proxy",
        description="Local CA MITM proxy that routes LLM traffic through the Section gateway.",
    )
    p.add_argument("--version", action="version", version=f"section-edge-proxy {__version__}")
    p.add_argument(
        "--ca-dir",
        type=Path,
        default=None,
    ...
    s_start.add_argument("--gateway", required=False, help="Section gateway base URL.")
    s_start.add_argument("--api-key", required=False, help="API key for the gateway.")
    s_start.add_argument(
        "--tenant",
        default=None,
        help="Section tenant id (default: 'default').",
    )
    s_start.add_argument(
        "--listen",
        default=None,
...
def cmd_start(args: argparse.Namespace) -> int:
    """Boot mitmproxy with the Section addon attached."""
    settings = build_settings(args)
    if not settings.api_key:
        print(
            "  WARNING: starting without an API key (SECTION_EDGE_API_KEY unset). "
    ...
    if not cert_path.exists() or not key_path.exists():
        print(
            "  ERROR: root CA not found. Run `section-edge-proxy install-ca` first.",
            file=sys.stderr,
        )
        return 3
    ...
    print(f"Section edge proxy v{__version__}")
    ...
    try:
        ...
        print(
            ...
            "Install with `pip install section-edge-proxy`.",
            file=sys.stderr,
        )
        return 4

    from .proxy import SectionAddon
    ...
    addon = SectionAddon(settings, gateway=GatewayClient(settings), status=status)
