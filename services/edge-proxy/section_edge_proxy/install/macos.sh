#!/bin/sh
# Section Edge Proxy — macOS trust-store install.
#
# Usage:
...
if [ "$MODE" = "--uninstall" ]; then
    echo "Removing Section CA from System keychain..."
    # `security delete-certificate` matches by CN. Fall back to OK if
    # no matching cert exists.
    security delete-certificate -c "Section Edge Proxy Local CA" "$KEYCHAIN" 2>/dev/null || true
    echo "OK: Section CA removed."
    exit 0
fi

echo "Installing Section CA into System keychain..."
