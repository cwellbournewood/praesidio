#!/bin/sh
# Praesidio gateway container entrypoint.
#
# Runs `alembic upgrade head` against the configured database (resolved from
# PRAESIDIO_DATABASE_URL or DATABASE_URL) before exec'ing the application,
# unless PRAESIDIO_AUTO_MIGRATE=0 (e.g. when a Helm pre-upgrade Job already
# applied migrations, or when running ad-hoc commands locally).

set -eu

if [ "${PRAESIDIO_AUTO_MIGRATE:-1}" = "1" ]; then
    echo "[entrypoint] running alembic upgrade head"
    cd /app
    # Alembic resolves the URL from PRAESIDIO_DATABASE_URL / DATABASE_URL via
    ...
    echo "[entrypoint] PRAESIDIO_AUTO_MIGRATE=0, skipping alembic"
fi
