...
--
-- This SQL file is a verbatim mirror of the `0001_initial` Alembic revision,
-- preserved ONLY so the Postgres container's docker-entrypoint-initdb.d
-- mechanism can initialise a brand-new data volume on first boot. All
-- ongoing schema changes happen through Alembic; do not hand-edit this
-- file. If you need to regenerate it from the Alembic head, run:
--
--   (cd services/gateway && alembic upgrade head --sql) > migrations/0001_init.sql
--
-- Idempotent (uses IF NOT EXISTS) so the init path can replay safely.
