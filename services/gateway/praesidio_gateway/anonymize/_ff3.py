...
This is the only FPE backend in Praesidio. It is deterministic, supports any
...
- Keys MUST be 128, 192 or 256 bits. The caller is responsible for HKDF
  derivation upstream (Praesidio derives per-tenant subkeys from
  ``PRAESIDIO_FPE_KEY``).
...
- The cipher is malleable in the standard FPE sense: an attacker who can
  ...
  per-entity (Praesidio uses ``HMAC(tenant_id || label || vault_epoch)``).
