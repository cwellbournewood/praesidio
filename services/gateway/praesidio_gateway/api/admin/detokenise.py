...
* Each tenant is rate-limited by
  ``PRAESIDIO_DETOK_RATE_LIMIT_PER_TENANT_RPM`` (default 30). The
...
class DetokeniseRequest(BaseModel):
    request_id: str = Field(
        ...
        min_length=1,
        max_length=128,
        description="The Praesidio request_id under which the placeholders were minted.",
    )
...
async def detokenise(
    ...
    rpm = int(getattr(settings, "praesidio_detok_rate_limit_per_tenant_rpm", 30))
    ...
    if not allowed:
        ...
        raise HTTPException(
            ...
            headers={
                ...
                "X-Praesidio-RateLimit-Limit": str(rpm),
                "X-Praesidio-RateLimit-Scope": "detokenise",
            },
        )
