...
def _block_response(prep: PreparedRequest, *, status_code: int = 403) -> Response:
    """Render a Praesidio block response with reason/severity headers.

    Headers always populated:
      x-praesidio-decision: "block"
      x-praesidio-request-id: <request id>
      x-praesidio-policy: <matched policy id or "">
      X-Praesidio-Reason: <human-readable reason or "blocked by policy">
      X-Praesidio-Severity: <severity from rule or "high">
    """
...
async def _handle_upstream_failure(state: AppState, prep: PreparedRequest, exc: Exception):
    ...
    if fail_mode == "open":
        return Response(
            content=json.dumps({"error": {"type": "praesidio_degraded", "message": str(exc)}}),
            status_code=502,
            media_type="application/json",
            headers={"x-praesidio-reason": "praesidio_degraded"},
        )
    return Response(
        content=json.dumps({"error": {"type": "praesidio_unavailable", "message": str(exc)}}),
        status_code=503,
        media_type="application/json",
        headers={"x-praesidio-reason": "praesidio_unavailable"},
    )
