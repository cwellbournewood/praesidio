...
POSTs each audit row as a single JSON document to ``PRAESIDIO_SIEM_WEBHOOK_URL``.

When ``PRAESIDIO_SIEM_WEBHOOK_SECRET`` is set, every POST carries an
``X-Praesidio-Signature: sha256=<hex>`` header computed as
``HMAC-SHA256(secret, raw_request_body)``. Receivers verify the signature to
ensure the event came from a trusted Praesidio instance.
    ...
    async def emit(self, rows: list[dict[str, Any]]) -> None:
        ...
        for row in rows:
            try:
                ...
                if self.secret:
                    headers["x-praesidio-signature"] = sign(self.secret, body)
