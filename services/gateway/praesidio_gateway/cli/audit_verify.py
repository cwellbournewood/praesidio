"""``praesidio-audit verify`` — walk the audit chain and detect tampering.
...
gateway (:mod:`praesidio_gateway.audit.chain`). Any mismatch is reported.

Exits ``0`` when the chain is intact, ``1`` when any row fails to verify.

Usage::

    praesidio-audit verify --tenant acme --since 24h
    praesidio-audit verify --tenant acme --since 1h --dsn $DATABASE_URL
"""
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="praesidio-audit",
        description="Praesidio audit-chain operator CLI.",
    )
