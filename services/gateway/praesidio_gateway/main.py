...
async def _lifespan(app: FastAPI):
    ...
    try:
        await state.policy_store.start(interval=settings.praesidio_policy_reload_seconds)
        ...
        raise
    app.state.praesidio = state
    log.info(
        "praesidio gateway ready",
        env=settings.praesidio_env,
...
def create_app() -> FastAPI:
    ...
    async def _request_id_mw(request: Request, call_next):
        ...
        response.headers["x-praesidio-latency-ms"] = str(elapsed_ms)
        REQUEST_TOTAL.labels(
    ...
    if settings.praesidio_env == "development":
        ...
        app.add_middleware(
            ...
            allow_headers=["*"],
            expose_headers=[
                "X-Praesidio-Decision",
                "X-Praesidio-Reason",
                "X-Praesidio-Severity",
                "X-Praesidio-Request-Id",
                "X-Praesidio-Policy",
                "X-Request-Id",
            ],
        )
    else:
        # In production, operators MUST set PRAESIDIO_CORS_ORIGINS to the
        # exact extension ID(s) they distribute.
        configured = [o.strip() for o in (settings.praesidio_cors_origins or "").split(",") if o.strip()]
        if configured:
            app.add_middleware(
                CORSMiddleware,
    ...
    if settings.praesidio_rate_limit_enabled:
        app.add_middleware(
            RateLimitMiddleware,
            rpm=settings.praesidio_rate_limit_rpm,
            redis_url=settings.redis_url,
            per_key_rpm=settings.praesidio_rate_limit_per_key_rpm,
            tpm_default=settings.praesidio_rate_limit_tpm_default,
    ...
    #
    #   from praesidio_gateway.api.vectors import register_connector
...
def run() -> None:
    parser = argparse.ArgumentParser("praesidio-gateway")
    ...
    uvicorn.run(
        "praesidio_gateway.main:app",
        host=settings.praesidio_host,
        port=settings.praesidio_port,
        log_level=settings.praesidio_log_level.lower(),
        access_log=False,
        proxy_headers=True,
        forwarded_allow_ips="*",
    )
