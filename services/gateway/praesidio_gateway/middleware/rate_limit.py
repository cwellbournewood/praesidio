...
2. ``apikey`` — N requests-per-minute per API-key fingerprint, controlled
   by ``PRAESIDIO_RATE_LIMIT_PER_KEY_RPM`` (0 disables). Useful when a
...
3. ``model`` — TPM (tokens-per-minute) bucket keyed by ``(tenant, model)``,
   ...
   The default is ``PRAESIDIO_RATE_LIMIT_TPM_DEFAULT``;
   ``PRAESIDIO_RATE_LIMIT_TPM_PER_MODEL`` (a JSON map) overrides per
...
header and increment ``praesidio_rate_limit_blocked_total{tenant,scope}``.
The response carries ``X-Praesidio-RateLimit-Scope: tenant|apikey|model``
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def _tenant_for(self, request: Request) -> str:
        ...
        t = request.headers.get("x-praesidio-tenant")
        if t:
    ...
    def _block_response(
        ...
        *,
        ...
        headers = {
            ...
            "X-Praesidio-RateLimit-Limit": str(limit),
            "X-Praesidio-RateLimit-Tenant": tenant,
            "X-Praesidio-RateLimit-Scope": scope,
        }
        if extra_headers:
    ...
    async def dispatch(self, request: Request, call_next):
            ...
            if fp is not None:
                ...
                if not allowed:
                    return self._block_response(
                        ...
                        extra_headers={"X-Praesidio-RateLimit-Key": fp},
                    )
        ...
        # ``X-Praesidio-Model-Hint`` from the parsed payload before the
        # upstream call, or simply rely on post-hoc consume.)
        model_hint = request.headers.get("x-praesidio-model-hint")
        if model_hint and self.tpm_capacity_for(model_hint) > 0:
            ...
            if not allowed:
                ...
                return self._block_response(
                    ...
                    extra_headers={"X-Praesidio-RateLimit-Model": model_hint},
                )
...
async def consume_tpm_after_upstream(
    ...
    if limiter is None:
        return
    tenant = request.headers.get("x-praesidio-tenant", "anonymous").strip() or "anonymous"
