...
raw ``(tokens_in, tokens_out)`` pair) and:
  ...
  * increments ``praesidio_tokens_in_total{tenant,model,route}``
  * increments ``praesidio_tokens_out_total{tenant,model,route}``
  * increments ``praesidio_cost_usd_total{tenant,model}`` (from
    :mod:`praesidio_gateway.obs.prices`)
