...
3. The ``praesidio_policy_reload_total{outcome="err"}`` counter increments.

On success:

- ``praesidio_policy_reload_total{outcome="ok"}`` increments;
- ``praesidio_policy_active_version{digest,policies}`` is reset to the new
