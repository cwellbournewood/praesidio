...
async def build_app_state(settings: Settings | None = None) -> AppState:
    ...
    webhook = SiemWebhookSink(
        settings.praesidio_siem_webhook_url,
        settings.praesidio_siem_webhook_secret,
        timeout=settings.praesidio_siem_webhook_timeout_seconds,
    )
    writer = AuditWriter(
        engine,
    ...
    store = PolicyStore(settings.praesidio_policy_bundle)
...
def get_state(request: Request) -> AppState:
    state = getattr(request.app.state, "praesidio", None)
    if state is None:
