...
1. **Write-time** — documents are run through an embedding model and the
   ...
   AND retained verbatim in metadata. Praesidio's :meth:`VectorConnector.
...
2. **Read-time** — semantic search returns documents the calling
   ...
   topic X and the model surfaced HR record Y" failure mode. Praesidio's
...
* :class:`praesidio_gateway.vectors.pgvector.PgVectorConnector` — uses
  asyncpg directly. Suitable for self-hosted Postgres + ``pgvector``.
* :class:`praesidio_gateway.vectors.qdrant.QdrantConnector` — uses
