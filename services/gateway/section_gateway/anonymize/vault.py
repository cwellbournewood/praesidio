...
The encryption key per-tenant is HKDF(SECTION_VAULT_KEY, salt=tenant).
The AAD on every record binds it to ``b"section|<tenant>|<request_id>"`` —
...
class TokenVault:
    ...
    def _aad(tenant: str, request_id: str) -> bytes:
        return b"section|" + tenant.encode() + b"|" + request_id.encode()
