...
def _block_response(prep: PreparedRequest, *, status_code: int = 403) -> Response:
    """Render an Anthropic-shaped block response with Section headers.

    Mirrors the OpenAI block path: emits ``X-Section-Reason`` and
    ``X-Section-Severity`` so SOC tooling has a uniform shape across
    provider routes.
    """
