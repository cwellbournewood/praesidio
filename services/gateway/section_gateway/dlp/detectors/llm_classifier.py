...
* ``SECTION_LLM_CLASSIFIER_URL`` — chat-completions endpoint (any
  OpenAI-compatible API: OpenAI, Azure, vLLM, llama.cpp server,
  Anthropic via a compatibility shim, etc.).
* ``SECTION_LLM_CLASSIFIER_MODEL`` — model id (e.g. ``gpt-4o-mini``).
* ``SECTION_LLM_CLASSIFIER_API_KEY`` — optional bearer token.
* ``SECTION_LLM_CLASSIFIER_THRESHOLD`` — float [0,1]; verdicts at or
  above this confidence raise a finding (default 0.5).
* ``SECTION_LLM_CLASSIFIER_TIMEOUT_SECONDS`` — request timeout
  (default 1.0s).
...
class LLMClassifierConfig:
    ...
    def from_env(cls) -> LLMClassifierConfig | None:
        url = (os.environ.get("SECTION_LLM_CLASSIFIER_URL") or "").strip()
        if not url:
            return None
        model = (os.environ.get("SECTION_LLM_CLASSIFIER_MODEL") or "gpt-4o-mini").strip()
        api_key = (os.environ.get("SECTION_LLM_CLASSIFIER_API_KEY") or "").strip() or None
        try:
            threshold = float(os.environ.get("SECTION_LLM_CLASSIFIER_THRESHOLD", "0.5"))
        except ValueError:
            threshold = 0.5
        threshold = min(1.0, max(0.0, threshold))
        try:
            timeout = float(os.environ.get("SECTION_LLM_CLASSIFIER_TIMEOUT_SECONDS", "1.0"))
        except ValueError:
            timeout = 1.0
