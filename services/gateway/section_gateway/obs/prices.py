...
Prices are operator-overridable via ``SECTION_PRICE_BOOK_JSON`` (a JSON
...
def _load_overrides() -> dict[str, Price]:
    raw = os.environ.get("SECTION_PRICE_BOOK_JSON", "").strip()
    if not raw:
        return {}
    try:
        ...
        _log.warning("SECTION_PRICE_BOOK_JSON is not valid JSON; ignoring overrides")
        return {}
    ...
    if p is None:
        ...
        if key and key not in _unknown_warned:
            ...
            _log.warning(
                ...
                "SECTION_PRICE_BOOK_JSON to override.",
                model,
            )
        return 0.0
