...
tenant bucket, surfaces the right ``X-Praesidio-RateLimit-Scope`` and
``Retry-After`` headers, and increments
``praesidio_rate_limit_blocked_total{scope="apikey"}``.
"""
...
from praesidio_gateway.middleware.rate_limit import RateLimitMiddleware
from praesidio_gateway.obs.metrics import RATE_LIMIT_BLOCKED_TOTAL
...
def test_per_key_bucket_trips_before_tenant() -> None:
    ...
    headers = {"X-API-Key": "secret-key-1", "X-Praesidio-Tenant": "tA"}
    ...
    assert r.headers.get("X-Praesidio-RateLimit-Scope") == "apikey"
    assert r.headers.get("Retry-After") is not None
    # Key fingerprint header surfaced for SRE debugging.
    assert r.headers.get("X-Praesidio-RateLimit-Key")
    body = r.json()["error"]
...
def test_different_keys_have_independent_buckets() -> None:
    ...
    client = TestClient(app)
    h1 = {"X-API-Key": "key-aaa", "X-Praesidio-Tenant": "tA"}
    h2 = {"X-API-Key": "key-bbb", "X-Praesidio-Tenant": "tA"}
    for _ in range(2):
...
def test_tenant_bucket_trips_independently_of_key() -> None:
    ...
    r = client.get("/echo", headers=headers)
    assert r.status_code == 429
    assert r.headers.get("X-Praesidio-RateLimit-Scope") == "tenant"
...
def test_per_key_uses_bearer_token() -> None:
    ...
    app = _make_app(rpm=10_000, per_key_rpm=2)
    client = TestClient(app)
    headers = {"Authorization": "Bearer bearer-key", "X-Praesidio-Tenant": "tA"}
    for _ in range(2):
        assert client.get("/echo", headers=headers).status_code == 200
    r = client.get("/echo", headers=headers)
    assert r.status_code == 429
...
def test_blocked_metric_increments_with_scope_label() -> None:
    ...
    client = TestClient(app)
    headers = {"X-API-Key": "metric-key", "X-Praesidio-Tenant": "tA"}
...
async def test_inmem_bucket_refills_over_time() -> None:
    ...
    from praesidio_gateway.middleware.rate_limit import _InMemoryLimiter
