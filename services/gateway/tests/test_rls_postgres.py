...
the RLS policies; this test confirms ``SET section.tenant_id`` actually
filters reads to the matching tenant.

Run only this test::

    SECTION_PG_DSN=postgresql+asyncpg://section:section@localhost/section \\
...
from section_gateway.audit.models import AuditEvent, Base
...
async def _ensure_schema_and_policies(engine) -> None:
    ...
    async with engine.begin() as conn:
        ...
        for table in ("audit_events", "lineage_nodes"):
            ...
            await conn.execute(
                text(f"DROP POLICY IF EXISTS section_tenant_isolation ON {table}")
            )
            await conn.execute(
                text(
                    f"CREATE POLICY section_tenant_isolation ON {table} "
                    f"USING (tenant_id = current_setting('section.tenant_id', true) "
                    f"OR current_setting('section.tenant_id', true) = '*') "
                    f"WITH CHECK (tenant_id = current_setting('section.tenant_id', true) "
                    f"OR current_setting('section.tenant_id', true) = '*')"
                )
            )
...
async def test_rls_isolates_tenants():
    dsn = _pg_dsn()
    if not dsn:
        pytest.skip("SECTION_PG_DSN not set — Postgres RLS test skipped")
    ...
    try:
        ...
        async with sm() as s:
            ...
            # the section role (which is a superuser in the test container)
            # bypasses RLS regardless of FORCE. The section_app_test role is
            # provisioned by .github/workflows/rls.yml.
            await s.execute(text("SET ROLE section_app_test"))
            await s.execute(text("SET section.tenant_id = 'A'"))
            res = await s.execute(text("SELECT tenant_id FROM audit_events"))
            assert [r[0] for r in res.fetchall()] == ["A"]

        async with sm() as s:
            await s.execute(text("SET ROLE section_app_test"))
            await s.execute(text("SET section.tenant_id = 'B'"))
            res = await s.execute(text("SELECT tenant_id FROM audit_events"))
        ...
        async with sm() as s:
            await s.execute(text("SET ROLE section_app_test"))
            await s.execute(text("SET section.tenant_id = '*'"))
            res = await s.execute(
                text("SELECT tenant_id FROM audit_events ORDER BY tenant_id")
            )
            assert [r[0] for r in res.fetchall()] == ["A", "B"]
    except Exception as exc:
        if "section_app_test" in str(exc):
            pytest.skip("section_app_test role not provisioned; skip")
        raise
    finally:
...
async def test_rls_no_tenant_setting_returns_nothing():
    """If section.tenant_id is unset, the policy evaluates to NULL and
